...
async def create_project(project: ProjectInit, db: Session = Depends(get_db)):
    """
...
async def sample_bandit(project_id: int, db: Session = Depends(get_db)):
    """
...
async def give_feedback( liked: bool, db: Session = Depends(get_db)):
    """
...
async def select_bandits(project_id: int, n: int = Query(1, gt=0), db: Session = Depends(get_db)):
    """
...
async def generate_bandit_plot(project_id: int, db: Session = Depends(get_db)):
    """
    ...
    for bandit in bandits:
        y = beta_distribution.pdf(x, bandit.alpha, bandit.beta)
